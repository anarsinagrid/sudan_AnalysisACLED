...
**Hypothesis**: I initially planned a three-part structure:
...
- **Reporting Lag**: My analysis showed that the "fog of war" in that first week meant many events were back-filled later or reported with lower precision.
- **Conclusion**: A "micro-history" would require granular qualitative data (interviews, ground reports) that purely event-based data couldn't sustain without creating misleading certainty.

---

## 2. Geography: The Density Trap

**What I tried**: I wanted to show the hardest-hit regions immediately. I generated `F4_admin1_event_concentration.png` to rank states by total violence.
...
- **The Solution**: I switched to the **Choropleth Map (Figure 6)** in the final report. It uses time-decay and specific event locations to show *where* violence lives, rather than just tallying it up on a leaderboard.

---

## 3. The Sexual Violence Narrative

**What I explored**: A dedicated section analyzing trends in violence against women.
...
**The Problem**: The raw data is a mess of naming conventions. I had "Military Forces of Sudan," "SAF," "Government of Sudan," "RSF," "Janjaweed," "Militias," etc.

**Attempt 1: The Raw Distribution**
I visualized the raw actor counts (`AX_01_actor_overall_distribution.png`).
...
**The Fix**: I implemented a rigorous **Actor Normalization** pipeline in the code, bucketing these dozens of names into: `SAF`, `RSF`, `Protest Groups`, and `Other Armed Groups`.

**Attempt 2: Behavioral Heatmaps**
Once normalized, I tried to find their "fingerprints" using heatmaps.
...
**The Insight**: These heatmaps confirmed my hypothesis—SAF and RSF behave differently. SAF correlates with "Explosions/Remote Violence" (Battles), while RSF correlates strongly with "Violence against civilians."
...
**What I tried**: To show the shift from "Protest" to "War," I built a **Slopegraph** (`F21_composition_slopegraph.png`).
...
- **The Solution**: I used **Stacked Density Plots (Figure 2)**. They show the *collapse* of protest volume and the *surge* of battle volume simultaneously, which is the emotional and structural truth of the story.
...
- I didn't just map every point.
- In the data cleaning pipeline, I filtered based on ACLED's `geo_precision` code.
...
- **Narrative friction**: Every time I slotted it in, it interrupted pacing instead of propelling it.

**Decision**: Parked. Kept here as a reminder that visual novelty can still lose to clarity.

---

*This document serves as my audit trail for the "Choice" to prioritize narrative clarity over data volume.*
